...
const followshipService = {
  followUser: (req, res, cb) => {
    if (req.user.id == req.body.id)
      return cb({ status: "400", message: "不能追蹤自己" });
    Promise.all([
      ...
      .then(([user, followship]) => {
        if (user === null)
          return cb({ status: "400", message: "用戶不存在，無法追蹤" });
        if (followship)
          return cb({
            status: "400",
            message: "您已經正在追蹤此用戶，不可重複追蹤",
          });
        Followship.create({
          followerId: req.user.id,
          followingId: req.body.id,
        }).then((followship) =>
          cb({ status: "200", message: "追蹤成功", id: followship.id })
        );
      })
      .catch((err) => {
        console.warn(err);
