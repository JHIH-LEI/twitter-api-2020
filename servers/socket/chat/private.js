      ...
      let [room, created] = await Room.findOrCreate({
        ...
        where: {
          [sequelize.Op.and]: sequelize.literal(
            `(Room.creatorId = ${loginUser.id} OR Room.creatorId =${targetUserId})AND (Room.joinerId =${loginUser.id} OR Room.joinerId = ${targetUserId})`
          ),
        },
        defaults: {
      ...
      socket.on("leave room", async () => {
        try {
          await RoomUser.destroy({
            where: { RoomId: room.id, UserId: loginUser.id },
          });
